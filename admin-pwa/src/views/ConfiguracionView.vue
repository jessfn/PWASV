  ...
  <div class="configuracion-container">
    ...
    <main class="main-content">      <header class="page-header">
      ...
      <div class="page-content">
        ...
        <!-- NUEVA SECCIÓN: Control de Modo Mantenimiento -->
        <div class="config-section">
          <h2>🔧 Control de Modo Mantenimiento</h2>
          <div class="config-card maintenance-card">
            <div class="maintenance-header">
              <div class="maintenance-icon" :class="{ 'active': modoMantenimiento }">
                <svg v-if="!modoMantenimiento" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              ...
              <div class="maintenance-info">
                <h3>{{ modoMantenimiento ? 'Modo Mantenimiento ACTIVO' : 'Sistema en Funcionamiento Normal' }}</h3>
                <p class="maintenance-status" :class="{ 'active': modoMantenimiento }">
                  {{ modoMantenimiento ? 'La PWA Super está en modo mantenimiento' : 'La PWA Super está funcionando normalmente' }}
                </p>
                <p v-if="mensajeMantenimiento" class="maintenance-message">
                  <strong>Mensaje:</strong> {{ mensajeMantenimiento }}
                </p>
              </div>
            </div>
            ...
            <div class="maintenance-controls">
              ...
              <div v-if="!modoMantenimiento" class="maintenance-message-input">
                <label for="maintenanceMessage">Mensaje personalizado (opcional):</label>
                <textarea 
                  id="maintenanceMessage"
                  v-model="mensajePersonalizado"
                  placeholder="Ej: Sistema en mantenimiento programado. Estimamos volver en 30 minutos."
                  rows="3"
                  maxlength="200"
                  class="maintenance-textarea"
                ></textarea>
                <div class="character-count">{{ mensajePersonalizado.length }}/200</div>
              </div>
              
              <div class="maintenance-actions">
                <button 
                  @click="verificarEstadoMantenimiento" 
                  class="maintenance-btn check-btn"
                  :disabled="procesandoMantenimiento"
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  {{ procesandoMantenimiento ? 'Verificando...' : 'Verificar Estado' }}
                </button>
                
                <button 
                  v-if="modoMantenimiento"
                  @click="forzarRecargaPWA" 
                  class="maintenance-btn reload-btn"
                  :disabled="procesandoMantenimiento"
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div class="maintenance-info-box">
              <h4>ℹ️ Información sobre el Modo Mantenimiento</h4>
              <ul>
                <li><strong>Activado:</strong> Los usuarios verán una pantalla verde con el mensaje de mantenimiento</li>
                <li><strong>Automático:</strong> La PWA Super se recarga automáticamente cuando se activa/desactiva</li>
                <li><strong>Tiempo real:</strong> Los cambios se reflejan inmediatamente en todas las sesiones</li>
                <li><strong>Acceso admin:</strong> Este panel administrativo sigue funcionando normalmente</li>
              </ul>
            </div>
          </div>
        </div>
...
// Variables para el modo mantenimiento
const modoMantenimiento = ref(false)
const procesandoMantenimiento = ref(false)
const mensajeMantenimiento = ref('')
const mensajePersonalizado = ref('Sistema en mantenimiento programado. Volveremos pronto.')
...
const verificarEstadoMantenimiento = async () => {
  try {
    console.log('🔍 Verificando estado de mantenimiento...')
    const resultado = await maintenanceService.verificarEstadoMantenimiento()
    
    if (resultado.success) {
      modoMantenimiento.value = resultado.enabled
      mensajeMantenimiento.value = resultado.message || ''
      
      console.log(`📋 Estado actual: ${resultado.enabled ? 'MANTENIMIENTO' : 'NORMAL'}`)
      if (resultado.simulated) {
        console.log('⚠️ Estado simulado (endpoint no disponible)')
      }
    }
  } catch (error) {
    console.error('❌ Error verificando mantenimiento:', error)
    mostrarMensaje('Error', 'Error al verificar el estado del mantenimiento: ' + error.error)
  }
}

const toggleMantenimiento = async () => {
  if (procesandoMantenimiento.value) return
  
  procesandoMantenimiento.value = true
  
  try {
    if (modoMantenimiento.value) {
      // Activar mantenimiento
      showConfirmation(
        '🔧 Activar Modo Mantenimiento',
        `¿Estás seguro de que quieres activar el modo mantenimiento?<br><br>
        <strong>Esto hará que:</strong><br>
        • La PWA Super muestre una pantalla de mantenimiento<br>
        • Los usuarios no puedan usar la aplicación<br>
        • Se recargue automáticamente en todas las sesiones<br><br>
        <strong>Mensaje:</strong> "${mensajePersonalizado.value}"`,
        async () => {
    ...
    modoMantenimiento.value = !modoMantenimiento.value
  } finally {
    procesandoMantenimiento.value = false
  }
}
...
const activarMantenimiento = async () => {
  try {
    ...
    const resultado = await maintenanceService.activarMantenimientoCompleto(
      mensajePersonalizado.value || 'Sistema en mantenimiento programado. Volveremos pronto.'
    )
    
    if (resultado.success) {
      mensajeMantenimiento.value = mensajePersonalizado.value
      
      mostrarMensaje(
        '✅ Modo Mantenimiento Activado', 
        `<div style="text-align: left;">
          <h4 style="color: #e67e22; margin-bottom: 15px;">🔧 Mantenimiento Activo</h4>
          <p><strong>Estado:</strong> La PWA Super está ahora en modo mantenimiento</p>
          <p><strong>Mensaje:</strong> "${mensajeMantenimiento.value}"</p>
          <hr style="margin: 15px 0;">
          <p style="font-size: 12px; color: #666;">
            ${resultado.simulated ? '⚠️ Modo simulado (endpoint no disponible)' : '✅ Configurado en el servidor'}
          </p>
          <p style="font-size: 12px; color: #666;">
            Los usuarios verán una pantalla de mantenimiento y la app se recargará automáticamente.
          </p>
        </div>`
      )
      
      // Simular notificación a PWA Super
      setTimeout(() => {
        console.log('📡 Simulando recarga automática de PWA Super...')
      }, 1000)
      
    } else {
      throw new Error(resultado.error || 'Error activando mantenimiento')
    }
  } catch (error) {
    console.error('❌ Error activando mantenimiento:', error)
    modoMantenimiento.value = false
    mostrarMensaje('❌ Error', 'Error al activar el modo mantenimiento: ' + (error.message || error.error || 'Error desconocido'))
  } finally {
    procesandoMantenimiento.value = false
  }
}
...
const desactivarMantenimiento = async () => {
  try {
    ...
    const resultado = await maintenanceService.desactivarMantenimientoCompleto()
    
    if (resultado.success) {
      mensajeMantenimiento.value = ''
      
      mostrarMensaje(
        '✅ Modo Mantenimiento Desactivado', 
        `<div style="text-align: left;">
          <h4 style="color: #27ae60; margin-bottom: 15px;">✅ Sistema Restaurado</h4>
          <p><strong>Estado:</strong> La PWA Super ha vuelto al funcionamiento normal</p>
          <p><strong>Acceso:</strong> Los usuarios pueden usar la aplicación normalmente</p>
          <hr style="margin: 15px 0;">
          <p style="font-size: 12px; color: #666;">
            ${resultado.simulated ? '⚠️ Modo simulado (endpoint no disponible)' : '✅ Configurado en el servidor'}
          </p>
          <p style="font-size: 12px; color: #666;">
            La PWA Super se recargará automáticamente y volverá a estar disponible.
          </p>
        </div>`
      )
      ...
      throw new Error(resultado.error || 'Error desactivando mantenimiento')
    }
  } catch (error) {
    console.error('❌ Error desactivando mantenimiento:', error)
    modoMantenimiento.value = true
    mostrarMensaje('❌ Error', 'Error al desactivar el modo mantenimiento: ' + (error.message || error.error || 'Error desconocido'))
  } finally {
    procesandoMantenimiento.value = false
  }
}

const forzarRecargaPWA = async () => {
  showConfirmation(
    '🔄 Forzar Recarga de PWA Super',
    '¿Quieres forzar la recarga de todas las sesiones de la PWA Super?<br><br>Esto actualizará inmediatamente el estado de mantenimiento en todos los dispositivos conectados.',
    async () => {
      try {
        mostrarMensaje('🔄 Recargando...', 'Enviando señal de recarga a la PWA Super...')
        
        // Simular envío de señal de recarga
        await new Promise(resolve => setTimeout(resolve, 2000))
        
        mostrarMensaje('✅ Recarga Enviada', 'Se ha enviado la señal de recarga a todas las sesiones de la PWA Super.')
      } catch (error) {
        mostrarMensaje('❌ Error', 'Error al enviar la señal de recarga: ' + error.message)
      }
    }
  )
}
...
.maintenance-message-input label {
  font-weight: 600;
  color: #2c3e50;
  font-size: 14px;
}
...
.maintenance-btn.check-btn {
  background: linear-gradient(135deg, #3498db 0%, #2980b9 100%);
  color: white;
  box-shadow: 0 4px 15px rgba(52, 152, 219, 0.3);
}

.maintenance-btn.check-btn:hover:not(:disabled) {
  background: linear-gradient(135deg, #2980b9 0%, #1e6091 100%);
  transform: translateY(-2px);
  box-shadow: 0 6px 20px rgba(52, 152, 219, 0.4);
}

.maintenance-btn.reload-btn {
  background: linear-gradient(135deg, #f39c12 0%, #e67e22 100%);
  color: white;
  box-shadow: 0 4px 15px rgba(243, 156, 18, 0.3);
}

.maintenance-btn.reload-btn:hover:not(:disabled) {
...
.maintenance-info-box ul {
  margin: 0;
  padding-left: 20px;
  color: #2c3e50;
}

.maintenance-info-box li {
  margin-bottom: 8px;
  line-height: 1.4;
}

.maintenance-info-box li:last-child {
  margin-bottom: 0;
}
