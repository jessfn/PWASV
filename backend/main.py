...
try:
    ...
    print("✅ Servicio de Push Notifications cargado correctamente")
except ImportError as e:
    PUSH_NOTIFICATIONS_ENABLED = False
    VAPID_PUBLIC_KEY = None
    print(f"⚠️ Push Notifications no disponible: {e}")
...
async def crear_notificacion(
    ...
    try:
        ...
        return {
            ...
            "push_enviadas": push_result.get("sent", 0)
        }
...
class PushSubscriptionCreate(BaseModel):
    """Modelo para crear una suscripción push"""
    usuario_id: int
    endpoint: str
    p256dh: str
    auth: str
...
class PushSubscriptionDelete(BaseModel):
    """Modelo para eliminar una suscripción push"""
    endpoint: str

@app.get("/push/vapid-public-key")
async def obtener_vapid_public_key():
    """Obtener la clave pública VAPID para suscripciones push"""
    try:
        if not PUSH_NOTIFICATIONS_ENABLED:
            raise HTTPException(status_code=503, detail="Push Notifications no disponible en este servidor")
        ...
        print(f"❌ Error obteniendo VAPID key: {e}")
        raise HTTPException(status_code=500, detail=str(e))

@app.post("/push/subscribe")
async def suscribir_push(subscription: PushSubscriptionCreate):
    """Suscribir un dispositivo para recibir push notifications"""
    try:
        if not conn:
            raise HTTPException(status_code=500, detail="No hay conexión a la base de datos")
        ...
        print(f"🔔 Suscribiendo dispositivo para usuario {subscription.usuario_id}")
        
        # Verificar que el usuario existe
        cursor.execute("SELECT id FROM usuarios WHERE id = %s", (subscription.usuario_id,))
        if not cursor.fetchone():
            raise HTTPException(status_code=404, detail="Usuario no encontrado")
        ...
        if existing:
            ...
            cursor.execute("""
                ...
                SET usuario_id = %s, p256dh = %s, auth = %s, 
                    device_info = %s, activa = TRUE, 
                    fecha_actualizacion = NOW(), ultimo_uso = NOW()
                WHERE endpoint = %s
                RETURNING id
            """, (
                ...
                subscription.p256dh, 
                subscription.auth,
                json.dumps(subscription.device_info) if subscription.device_info else None,
                subscription.endpoint
            ))
            subscription_id = cursor.fetchone()[0]
            print(f"✅ Suscripción actualizada: {subscription_id}")
        else:
            # Crear nueva suscripción
            cursor.execute("""
                INSERT INTO push_subscriptions 
                (usuario_id, endpoint, p256dh, auth, device_info, activa)
            ...
            subscription_id = cursor.fetchone()[0]
            print(f"✅ Nueva suscripción creada: {subscription_id}")
...
async def cancelar_suscripcion_push(data: PushSubscriptionDelete):
    """Cancelar una suscripción push"""
    try:
        if not conn:
            raise HTTPException(status_code=500, detail="No hay conexión a la base de datos")
        
        print(f"🔕 Cancelando suscripción: {data.endpoint[:50]}...")
        ...
        if result:
            conn.commit()
            print(f"✅ Suscripción eliminada: {result[0]}")
...
async def obtener_suscripciones_usuario(usuario_id: int):
    ...
    try:
        ...
        cursor.execute("""
            SELECT id, endpoint, device_info, activa, fecha_creacion, ultimo_uso
        ...
        for row in results:
            subscriptions.append({
                "id": row[0],
                "endpoint": row[1][:50] + "...",  # Truncar por seguridad
                "device_info": row[2],
                "activa": row[3],
...
async def enviar_push_a_destinatarios(notificacion_id: int, titulo: str, subtitulo: str = None, 
    ...
    try:
        ...
        for sub in subscriptions:
            estado = "enviado" if result["sent"] > 0 else "fallido"
            cursor.execute("""
                INSERT INTO push_notification_logs 
                (notificacion_id, usuario_id, estado)
                VALUES (%s, %s, %s)
            """, (notificacion_id, sub.usuario_id, estado))
